...
func Start(e *echo.Echo, port string, autoTls bool, cert string, pkey string) {
	if autoTls {
		startAutoTLS(e, port, cert, pkey)
		return
	}
...
func startAutoTLS(e *echo.Echo, port string, cert string, pkey string) {
	autoTLSManager := autocert.Manager{
		Prompt: autocert.AcceptTOS,
		// Cache certificates to avoid issues with rate limits (https://letsencrypt.org/docs/rate-limits)
		Cache: autocert.DirCache("/var/www/.cache"),
		HostPolicy: autocert.HostWhitelist(envs.DOMAIN),
	}
	s := http.Server{
		Addr:    port,
		Handler: e,
